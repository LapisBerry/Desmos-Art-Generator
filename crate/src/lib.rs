    ...
    for node in doc.descendants() {
        if node.tag_name().name() == "path" {
            if let Some(d) = node.attribute("d") {
                path_to_desmos(d, &mut equations);
            }
        }
    }
...
fn path_to_desmos(d: &str, eqs: &mut Vec<String>) {
    ...
    for seg in PathParser::from(d) {
        ...
        match seg {
            PathSegment::MoveTo { abs, x, y } => {
                cur = abs_pt(abs, cur, x, y);
                start = cur;
                prev_c2 = None;
            }
            PathSegment::LineTo { abs, x, y } => {
                let p = abs_pt(abs, cur, x, y);
                eqs.push(line_eq(cur, p));
                cur = p;
                prev_c2 = None;
            }
            PathSegment::HorizontalLineTo { abs, x } => {
                let p = (if abs { x } else { cur.0 + x }, cur.1);
                eqs.push(line_eq(cur, p));
                cur = p;
                prev_c2 = None;
            }
            PathSegment::VerticalLineTo { abs, y } => {
                let p = (cur.0, if abs { y } else { cur.1 + y });
                eqs.push(line_eq(cur, p));
                cur = p;
                prev_c2 = None;
            }
            PathSegment::CurveTo { abs, x1, y1, x2, y2, x, y } => {
                let c1 = abs_pt(abs, cur, x1, y1);
                let c2 = abs_pt(abs, cur, x2, y2);
                let p = abs_pt(abs, cur, x, y);
            ...
            PathSegment::SmoothCurveTo { abs, x2, y2, x, y } => {
                ...
                let c1 = prev_c2
                    .map_or(cur, |pc2| (2.0 * cur.0 - pc2.0, 2.0 * cur.1 - pc2.1));
                let c2 = abs_pt(abs, cur, x2, y2);
                let p = abs_pt(abs, cur, x, y);
                eqs.push(cubic_eq(cur, c1, c2, p));
                prev_c2 = Some(c2);
...
#[inline]
fn abs_pt(abs: bool, cur: (f64, f64), x: f64, y: f64) -> (f64, f64) {
